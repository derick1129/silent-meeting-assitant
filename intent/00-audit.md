...
| **4. Silent Lip Reading Pipeline** | **WORKING (Dual-Path: Geometric + VSR)** | 1. **Instant Path (0 ms):** In-browser MediaPipe FaceMesh tracks 40 lip landmarks calculating Lip Aspect Ratio (LAR) state machine.<br>2. **Pretrained VSR Path:** Local Hugging Face `MahmoodAnaam/MSP-VSR` CTC model decodes short video clips into controlled meeting intents. |
...
| **Audio Pipeline** | 16kHz Linear PCM live audio streaming via WebSocket to Deepgram Nova-2 with pre-buffer queue. | Add speaker diarization if multi-speaker separation is required. |
| **Lip Reading** | 1. In-browser MediaPipe FaceMesh LAR kinematics tracker running at 30 FPS.<br>2. Local pretrained `MahmoodAnaam/MSP-VSR` CTC model decoding WebM clips. | Optimize VSR inference with ONNX runtime or TensorRT for sub-second clip decoding. |
